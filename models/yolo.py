...
class Detect(nn.Module):
    ...
    def __init__(self, nc=1, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.no = nc + 5 +10 # number of outputs per anchor  **** add landmarks x y num  *****
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                """
                commented to add landmarks forward compute
                y = x[i].sigmoid()
                if self.inplace:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    no = na * (nc + 5 +10)  # adding landmarks
